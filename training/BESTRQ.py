...
def brq_split_in_half_mask_collate_fn(
    samples_lst, get_out_len_fn, mask_prob, mask_length, n_mels
):
    """This creates a batch from a list of samples and also creates
    the mask that will be used to mask the inputs of BEST-RQ.
    ...
    for sample in samples_lst:
        ...
        half_len = sig.size(-1) // 2
        first_half = sig[:half_len]
        second_half = sig[half_len:]
        
        # append both to list
        wav_lst.append(first_half)
        wav_lst.append(second_half)

        # get out_len for both
        latent_length_lst.append(get_out_len_fn(torch.as_tensor(first_half.size(-1))).item())
        latent_length_lst.append(get_out_len_fn(torch.as_tensor(second_half.size(-1))).item())
