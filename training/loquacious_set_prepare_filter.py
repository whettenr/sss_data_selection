...
# def filter_dataset_by_ids_old(ds, ids_to_keep):
#     """Filters each split of the dataset based on a list of IDs."""
#     print("STARTING FILTER ...")

#     id_set = set(ids_to_keep)
#     filtered_dataset = ds.filter(lambda x: x['ID'] in id_set)

#     return filtered_dataset

#     # id_set = set(ids_to_keep)
#     # table = dataset_dict.data
#     # id_array = pa.array(list(id_set))
#     # batch_size = 1_000
#     # batches = []
#     # for start in tqdm(range(0, table.num_rows, batch_size)):
#     #     end = min(start + batch_size, table.num_rows)
#     #     batch = table.slice(start, end)
#     #     mask = pc.is_in(batch["ID"], value_set=id_array)
#     #     batches.append(batch.filter(mask))

#     # id_set = set(ids_to_keep)

#     # table = dataset_dict.data
#     # mask = pc.is_in(table["ID"], value_set=pa.array(list(id_set)))
#     # filtered_table = table.filter(mask)
#     # filtered_dataset = dataset_dict.from_arrow(filtered_table)
